...
module "k8s_addons" {
  ...
  source         = "./modules/kubernetes-addons"    #"./EKS-Blueprint/modules/kubernetes-addons"
  ...
  ingress_nginx_helm_config = {
    ...
    values = [
      templatefile("${path.module}/modules/nginx_ingress/nginx_ingress.yaml", {
...
resource "helm_release" "internal_nginx" {
  ...
  values = [
    templatefile("${path.module}/modules/internal_nginx_ingress/ingress.yaml", {
      enable_service_monitor = var.service_monitor_crd_enabled
    })
  ]
}
